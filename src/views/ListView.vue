<template>
    <div>
      <div v-if="!error">
        <SubsList v-if="loaded" :class="style" :listIcons="icons" :listUsers="userlist" :listPages="pages" :listAmount="parseInt(paginationAmount)"/>
  ...
  data(){
    ...
    methods:{
      localDataCheck(){
        ...
        if(this.auth){
          if(parseInt(localStorage.twSubsPagAmount)){
            this.paginationAmount = parseInt(localStorage.twSubsPagAmount)
          }
          console.log(this.paginationAmount)
